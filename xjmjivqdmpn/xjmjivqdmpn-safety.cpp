...
// as always, use Release x86 configuration to compile
// GDI Only version
#include <windows.h>
#include <tchar.h>
#include <ctime>
...
{
	if (MessageBoxW(NULL, L"This is a GDI Only, Run?", L"xjmjivqdmpn.exe (better by mrsuperbuddy) (GDI Only)", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
	{
		ExitProcess(0);
	}
	else
	{
		if (MessageBoxW(NULL, L"Are you sure? It will still move your mouse and it contains flashing lights - NOT for epilepsy", L"F̷i̷n̸a̴l̴ ̴W̸a̴r̸n̷i̸n̷g̷ - xjmjivqdmpn.exe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
		{
			ExitProcess(0);
		}
		else
